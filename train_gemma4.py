...
Troubleshooting (4-bit on T4 / Colab):
    A ``CUDA error: device-side assert triggered`` during ``train()`` is often
    reported asynchronously inside ``transformers`` generation (the traceback may
    point at ``_has_unfinished_sequences`` or sampling). For Gemma 4 E2B + QLoRA,
    use **FP16** for ``bnb_4bit_compute_dtype`` and the loaded model ``dtype``,
